...
function calculateBill() {
    const riceQty = parseInt(document.getElementById('rice').value) || 0;
    const wheatFlourQty = parseInt(document.getElementById('wheat-flour').value) || 0;
    const sugarQty = parseInt(document.getElementById('sugar').value) || 0;

    const ricePrice = 60;
    const wheatFlourPrice = 40;
    const sugarPrice = 50;

    totalBill = (riceQty * ricePrice) + (wheatFlourQty * wheatFlourPrice) + (sugarQty * sugarPrice);
...
function generateCertificate() {
    ...
    const certificate = `
        <div class="certificate">
            ...
            <p><h2>Presented by BigBasket</h2></p>
        </div>
    `;
